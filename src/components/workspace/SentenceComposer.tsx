...
import { ChevronDown, Info, Lightbulb, AlertTriangle, Wifi, WifiOff } from 'lucide-react';
...
import MACDIllustration from '../entry/MACDIllustration';
...
const SentenceComposer: React.FC<SentenceComposerProps> = ({
  ...
  // Get friendly labels (no "component" jargon)
  ...
  const getSubjectDisplayLabel = (subject: Subject): string => {
    if (subject.kind === "price") {
      return `${(subject.source || "close").toUpperCase()} price`;
    }
    if (subject.kind === "indicator") {
      // Friendly labels without "component" jargon
      const componentLabels: Record<string, string> = {
        line: `${value.left.name} line`,
        macd: "MACD line",
        signal: "Signal line", 
        histogram: "Histogram",
        zero: "Zero line (0)",
      ...
      return componentLabels[subject.component] || `${value.left.name} ${subject.component}`;
    }
  ...
  const getTargetDisplayLabel = (target: Target): string => {
    if (target.kind === "component") {
      ...
      return componentLabels[target.component!] || target.component!;
    }
    if (target.kind === "value") {
      return value.right.type === "value" ? value.right.value.toString() : "Value";
    }
        ...
        <div className="text-lg text-gray-900 leading-relaxed">
          ...
          <div className="inline-block relative group">
            <button className="inline-flex items-center space-x-1 px-3 py-1 bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-800 rounded-full text-sm font-medium transition-colors">
              <span>{value.subject ? getSubjectDisplayLabel(value.subject) : 'Which line?'}</span>
          ...
          <div className="inline-block relative group">
            <button className="inline-flex items-center space-x-1 px-3 py-1 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-800 rounded-full text-sm font-medium transition-colors">
              <span>
                {value.target ? getTargetDisplayLabel(value.target) : 'Compare to'}
              </span>
              <ChevronDown className="w-3 h-3" />
            </button>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          ...
          <div className="text-sm text-blue-700">
            {sentenceLoading ? (
              ...
              // Fallback sentence with friendly labels
              `${getSubjectDisplayLabel(value.subject || { kind: "indicator", component: "line" })} ${getOperatorLabel(value.op)} ${value.target ? getTargetDisplayLabel(value.target) : 'target'} on ${value.timeframe}`
            )}
          </div>
        </div>
